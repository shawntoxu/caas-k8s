...
def list_ns():
    url = (SERVER_URL + list_namespaces) % SERVER
...
@app.route(rc_name_url, methods=['GET', 'POST', 'DELETE','PUT'])
def op_rc(namespace=None,rc_name=None):
    ...
    if request.method == 'DELETE':
        ...
        rc_data=json.loads(request.data)
        size=rc_data['spec']['replicas'] 
        headers = {'Content-Type': 'application/strategic-merge-patch+json'}
        patch_data = '{"spec":{"replicas":%s}}' % size
        r = requests.patch(url, headers=headers, data=patch_data)
